...
/* === 1%-sprong configuratie === */
/* Zet op 1 voor normale +1 stappen (aanbevolen voor echt minen). */
/* Zet op 100 voor 1%-sprongen (slaat 99% van het werk over).    */
#define EXTRANONCE2_STEP_DIVISOR 100

static inline uint64_t extranonce2_range(uint8_t e2_len)
{
    /* Retourneert 2^(8*e2_len), of 0 als de waarde niet in uint64_t past (e2_len >= 8). */
    if (e2_len == 0) return 0;
    if (e2_len >= 8)  return 0;   /* 2^64 past niet in uint64_t */
    return (uint64_t)1 << (8 * e2_len);
}

static inline uint64_t extranonce2_step(uint64_t range)
{
    if (EXTRANONCE2_STEP_DIVISOR <= 1) {
        return 1;
    }
    if (range == 0) {
        /* 8 bytes: gebruik UINT64_MAX als benadering van 2^64 - 1 */
        return UINT64_MAX / EXTRANONCE2_STEP_DIVISOR;
    }
    /* Naar boven afronden zodat we exact 100 unieke waarden dekken. */
    return (range + EXTRANONCE2_STEP_DIVISOR - 1) / EXTRANONCE2_STEP_DIVISOR;
}
...
void create_jobs_task(void *pvParameters)
{
    ...
    ESP_LOGI(TAG, "extranonce2 step: 1/%d van bereik (%s)",
             EXTRANONCE2_STEP_DIVISOR,
             EXTRANONCE2_STEP_DIVISOR <= 1 ? "+1 (volledig)" : "1%-sprongen");
    ...
    while (1) {
        ...
        if (miner_job_is_rollable(current_work)) {
            /* === extranonce2 verplaatsing === */
            uint8_t  e2_len = current_work->extranonce2_len;
            uint64_t range  = extranonce2_range(e2_len);
            uint64_t step   = extranonce2_step(range);

            if (step == 0) step = 1;   /* veiligheidsnet */

            if (range != 0 && extranonce_2 > range - step) {
                /* Volledige ronde voltooid: reset naar 0. */
                ESP_LOGW(TAG, "extranonce2 ronde voltooid (step=%" PRIu64 "), reset naar 0", step);
                extranonce_2 = 0;
            } else {
                extranonce_2 += step;
            }

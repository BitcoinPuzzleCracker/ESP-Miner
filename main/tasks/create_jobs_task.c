...
/* ---------------------------------------------------------------
 * Hulpfunctie: bouw werk vanuit miner_job
 * extranonce2 blijft 0 (bewuste keuze)
 * --------------------------------------------------------------- */
                                         ...
                                         uint32_t current_version)
{
    ...
    if (job->type == JOB_TYPE_SV2_STANDARD) {
        ...
        if (e2_len > 0) {
            bin2hex(extranonce_2_bin, e2_len, extranonce_2_str,
...
/* ---------------------------------------------------------------
 * Hoofdtaak: create_jobs_task
 * Met hardware version rolling — ASIC rolt zelf
 * --------------------------------------------------------------- */
void create_jobs_task(void *pvParameters)
{
    ...
    uint64_t extranonce_2 = 0;              // blijft 0
    ...
    ESP_LOGI(TAG, "NerdOctaxe 13T Ultimate Firmware Ready! (Slot Notifier + Smart Job ID Check)");
    ...
    while (1) {
        ...
        if (notified == pdTRUE) {
            ...
            if (new_work) {
                if (strcmp(last_dispatched_job_id, new_work->job_id) != 0) {
                    is_new_job_id = true;
                    strncpy(last_dispatched_job_id, new_work->job_id,
                            sizeof(last_dispatched_job_id) - 1);
            ...
            if (new_work) {
                ESP_LOGI(TAG, "New Work Activated (slot %lu) %s (type %d, new_id: %s)",
                         ...
                         new_work->type, is_new_job_id ? "true" : "false");
            ...
            /* -----------------------------------------------------------
             * KERN: als hardware version rolling actief is en de job is
             * al verzonden, doe NIETS. De ASIC rolt zelf door.
             * ----------------------------------------------------------- */
        ...
        if (current_work != NULL) {
            ...
            /* Alleen als ASIC NIET zelf rolt: versie verhogen */
            if (!GLOBAL_STATE->DEVICE_CONFIG.family.asic.hardware_version_rolling) {

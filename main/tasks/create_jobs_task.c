...
void create_jobs_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        if (miner_job_is_rollable(current_work)) {
            ...
            // Software version rolling with random bits within the mask
            ...
            uint32_t base_version = current_work->version;
            
            for (int i = 0; i < midstates; i++) {
                uint32_t random_bits = esp_random() & mask;
                current_version = (base_version & ~mask) | random_bits;
            }
        }
        timeout_ms = ASIC_get_asic_job_frequency_ms(GLOBAL_STATE);
    }

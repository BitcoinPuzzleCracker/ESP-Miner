...
/* ============================================================
 *  EN2 STAPGROOTTE CONFIGURATIE
 * ============================================================
 *  - EN2_STEP_MODE 0 = normale +1 (echte mining, 100% bereik)
 *  - EN2_STEP_MODE 1 = grote stappen (testen, snel door bereik)
 *
 *  EN2_STEP_DIVISOR bepaalt in hoeveel stappen het hele bereik
 *  wordt doorlopen als EN2_STEP_MODE == 1.
 *    bv. 100  -> bereik in 100 stappen
 *    bv. 1000 -> bereik in 1000 stappen
 * ============================================================ */
#define EN2_STEP_MODE       1
#define EN2_STEP_DIVISOR    100ULL

static uint64_t compute_en2_max(uint32_t extranonce2_len)
{
    if (extranonce2_len >= 8) {
        return UINT64_MAX;
    }
    if (extranonce2_len == 0) {
        return 0;
    }
    return (1ULL << (8 * extranonce2_len)) - 1ULL;
}

static uint64_t compute_en2_step(uint32_t extranonce2_len)
{
#if EN2_STEP_MODE == 0
    (void)extranonce2_len;
    return 1ULL;
#else
    uint64_t max_en2 = compute_en2_max(extranonce2_len);
    uint64_t step = max_en2 / EN2_STEP_DIVISOR;
    if (step == 0) step = 1ULL;
    return step;
#endif
}
...
void create_jobs_task(void *pvParameters)
{
...
#if EN2_STEP_MODE == 0
    ESP_LOGI(TAG, "EN2 step mode: +1 (full range, real mining)");
#else
    ESP_LOGW(TAG, "EN2 step mode: DIVISOR=%llu (TEST MODE - most of range skipped!)",
             (unsigned long long)EN2_STEP_DIVISOR);
#endif
    ...
    while (1) {
        ...
        if (miner_job_is_rollable(current_work)) {
            /* ---------- EN2 ROLLING MET CONFIGUREERBARE STAP ---------- */
            uint32_t e2_len = current_work->extranonce2_len;
            uint64_t max_en2 = compute_en2_max(e2_len);
            uint64_t step    = compute_en2_step(e2_len);

            uint64_t next_en2 = extranonce_2 + step;

            /* Overflow of voorbij max? -> reset naar begin van bereik */
            if (next_en2 > max_en2 || next_en2 < extranonce_2) {
                ESP_LOGD(TAG, "en2 range exhausted (len=%u, max=0x%llX), wrapping to 0",
                         (unsigned)e2_len, (unsigned long long)max_en2);
                extranonce_2 = 0;
                /* Optioneel: hier even pauzeren tot nieuwe pool-job
                 * vTaskDelay(pdMS_TO_TICKS(50));
                 */
            } else {
                extranonce_2 = next_en2;
            }
            /* ---------------------------------------------------------- */

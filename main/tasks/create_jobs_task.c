...
// ==========================================
// YENİ: Non-Linear & Akıllı Maske Fonksiyonları
// ==========================================
...
static uint32_t generate_smart_nonlinear_version(uint32_t base_version, uint32_t version_mask, uint32_t *lfsr_state) {
    if (version_mask == 0) {
        return base_version;
    }
    uint32_t random_step = xorshift32(lfsr_state);
    uint32_t masked_current = base_version & version_mask;
    return (base_version & ~version_mask) | ((masked_current + random_step) & version_mask);
}
...
void create_jobs_task(void *pvParameters)
{
    ...
    ESP_LOGI(TAG, "ASIC Ready! (Zero-Extranonce2 + Smart Non-Linear BIP320 + Auto-Job-Update)");
...
static void generate_work_sv2(GlobalState *GLOBAL_STATE, sv2_job_t *sv2_job, double difficulty)
{
    ...
    if (version_mask != 0) {
        // Non-linear state seed for this job
        uint32_t lfsr_state = base_version ^ sv2_job->job_id;

        uint32_t rolled_version = generate_smart_nonlinear_version(base_version, version_mask, &lfsr_state);
        memcpy(midstate_data, &rolled_version, 4);
        midstate_sha256_bin(midstate_data, 64, midstate);
        reverse_32bit_words(midstate, next_job->midstate1);

        rolled_version = generate_smart_nonlinear_version(rolled_version, version_mask, &lfsr_state);
        memcpy(midstate_data, &rolled_version, 4);
        midstate_sha256_bin(midstate_data, 64, midstate);
...
static void generate_work_sv2_ext(GlobalState *GLOBAL_STATE, sv2_ext_job_t *ext_job, double difficulty)
{
    ...
    if (version_mask != 0) {
        // Non-linear state seed for this extended job
        uint32_t lfsr_state = base_version ^ ext_job->job_id;
